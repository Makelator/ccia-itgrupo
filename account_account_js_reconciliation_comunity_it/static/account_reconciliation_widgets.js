...
var abstractReconciliation = Widget.extend(ControlPanelMixin, {
    ...
    init: function(parent, context) {
        ...
        this.map_type_document_id_code = {};
        this.map_type_document_id_search_id = {};
        this.map_type_document_id_p1 = {};
        this.map_type_document_id_p2 = {};
        ...
        this.create_form_fields = {
            ...
            type_document_it: {
                id: "type_document_it",
                index: 25,
                corresponding_property: "type_document_it",
                label: _t("Tipo de Documento"),
                required: false,
                constructor: FieldMany2One,
                field_properties: {
                    relation: "einvoice.catalog.01",
                    string: _t("Tipo de Documento"),
                    type: "many2one",
                },
            },
            nro_comprobante_mod: {
                id: "nro_comprobante_mod",
                index: 30,
                corresponding_property: "nro_comprobante_mod",
                label: _t("Nro Comprobante"),
                required: false,
                constructor: FieldChar,
                field_properties: {
                    string: _t("Nro Comprobante"),
                    type: "char",
                },
            },
        };
    ...
    start: function() {
        ...
        return $.when(this._super()).then(function(){
            ...
            deferred_promises.push(new Model("einvoice.catalog.01")
                .query(['id', 'code'])
                .all().then(function(data) {
                    _.each(data, function(o) { self.map_type_document_id_code[o.id] = o.code });
                })
            );
            ...
            deferred_promises.push(new Model("einvoice.catalog.01")
                .query(['id', 'code'])
                .all().then(function(data) {
                    _.each(data, function(o) { self.map_type_document_id_search_id[o.code] = o.id });
                })
            );

            deferred_promises.push(new Model("einvoice.catalog.01")
                .query(['id', 'code','n_serie','n_documento'])
                .all().then(function(data) {
                    _.each(data, function(o) { self.map_type_document_id_p1[o.id] = o.n_serie });
                })
            );
            ...
            deferred_promises.push(new Model("einvoice.catalog.01")
                .query(['id', 'code','n_serie','n_documento'])
                .all().then(function(data) {
                    _.each(data, function(o) { self.map_type_document_id_p2[o.id] = o.n_documento });
                })
            );
            ...
            _(data).each(function(datum){
                var preset = {
                    ...
                    lines: [{
                        ...
                        amount: datum.amount,
                        tax_id: datum.tax_id,
                        type_document_it: datum.type_document_it,
                        nro_comprobante: datum.nro_comprobante,
...
var abstractReconciliationLine = Widget.extend({
    ...
    init: function(parent, context) {
        ...
        this.map_type_document_id_code = this.getParent().map_type_document_id_code;
        this.map_type_document_id_search_id = this.getParent().map_type_document_id_search_id;
        this.map_type_document_id_p1 = this.getParent().map_type_document_id_p1;
        this.map_type_document_id_p2 = this.getParent().map_type_document_id_p2;
        this.is_valid = true;
    ...
    updateAccountingViewCreatedLines: function() {
        ...
        _(self.getCreatedLines()).each(function(line){
            ...
            //line.nro_comprobante = document.getElementsByClassName('create_nro_comprobante')[0].children[1].children[0].value;
            ...
            if (line.type_document_it)
            {
                line.type_document_itvisual = self.map_type_document_id_code[line.type_document_it];  
            }
            line.nro_comprobante = line.nro_comprobante_mod;

            if (line.nro_comprobante && line.type_document_it)
            {
                var cantidad_1 = self.map_type_document_id_p1[line.type_document_it];
                var cantidad_2 = self.map_type_document_id_p2[line.type_document_it];
                var tmpnro_com = line.nro_comprobante.split("-");
                if (tmpnro_com.length === 2 )
                {
                    var txt_p1 = tmpnro_com[0];
                    if (tmpnro_com[0].length < cantidad_1)
                    {
                        txt_p1 = '0'.repeat(cantidad_1 - tmpnro_com[0].length) + tmpnro_com[0];
                    }

                    var txt_p2 = tmpnro_com[1];
                    if (tmpnro_com[1].length < cantidad_2)
                    {
                        txt_p2 = '0'.repeat(cantidad_2 - tmpnro_com[1].length) + tmpnro_com[1];
                    }
                    line.nro_comprobante = txt_p1 + '-' + txt_p2;
                }
                if (tmpnro_com.length === 1 )
                {
                    var txt_p1 = tmpnro_com[0];
                    if (tmpnro_com[0].length < cantidad_2)
                    {
                        txt_p1 = '0'.repeat(cantidad_2 - tmpnro_com[0].length) + tmpnro_com[0];
                    }                    
                    line.nro_comprobante = txt_p1;
                }
            }
    ...
    prepareCreatedMoveLinesForPersisting: function(lines) {
        ...
        return _.collect(lines, function(line) {
            ...
            if (line.type_document_it) dict['type_document_it'] = line.type_document_it;
            if (line.nro_comprobante) dict['nro_comprobante'] = line.nro_comprobante;
            return dict;
        });
    },
...
var bankStatementReconciliationLine = abstractReconciliationLine.extend({
    ...
    prepareCounterpartAMLDict: function(line) {
        return {
            ...
            type_document_it: this.map_type_document_id_search_id[line.type_document_it],
        };
    },
