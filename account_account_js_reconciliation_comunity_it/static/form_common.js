...
var CompletionFieldMixin = {
    ...
    get_search_result: function(search_val) {
        ...
        {
            if (this.field.relation == "account.account" )
            {
                if (this.view.model != "account.account" )
                {
                    flag_v = 2;
                }
            }    
        }
        var toop = self.build_domain();

        if (flag_v == 2)
        {
          
                if (toop.__domains[0].length != 0)
                {
                    console.log(typeof toop.__domains[0]);
                    if (typeof toop.__domains[0] == "object")
                    {
                        var elem = ['user_type_id.type','!=','view']
                        console.info(elem)
                        toop.__domains[0].push(elem);
                        
                    }
                    else{
                        toop.__domains[0] = (toop.__domains[0]).substring(0,toop.__domains[0].length-1) + ",('user_type_id.type','!=', 'view')]";  
                    }
                }
                else
                {
                 console.log(toop.__domains);
                 console.log(toop.__domains[0]);
                 //toop.__domains[0] = "[('user_type_id.type','!=', 'view')]";
                }
        ...
        if (this.field.type == "many2one")
        {
            if (this.field.relation == "account.analytic.account" )
            {
                if (this.view.model != "account.analytic.account" )
                {
                    flag_v = 2;
                }
            }    
        ...
        if (flag_v == 2)
        {
                ...
                if (toop.__domains[0].length != 0)
                {
                    ...
                    if (typeof toop.__domains[0] == "object")
                    {
                        ...
                        toop.__domains[0] = (toop.__domains[0]).substring(0,toop.__domains[0].length-1) + ",('type','!=', 'view')]";  
                    }
                }
                else
                {
        ...
        return this.orderer.add(dataset.name_search(
                search_val, new data.CompoundDomain(toop, exclusion_domain),
