...
function format_value (value, descriptor, value_if_empty) {
    ...
    switch (descriptor.widget || descriptor.type || (descriptor.field && descriptor.field.type)) {
        ...
        case 'datetime':
            if (typeof(value) == "string"){                
                value = moment(time.auto_str_to_date(value)).utc();               }
            else {               
...
function parse_value (value, descriptor, value_if_empty) {
    ...
    switch (descriptor.widget || descriptor.type || (descriptor.field && descriptor.field.type)) {
        ...
        case 'datetime':
            var datetime = moment(value, [date_pattern + ' ' + time_pattern, date_pattern_wo_zero + ' ' + time_pattern_wo_zero, moment.ISO_8601], true).add(-(new Date().getTimezoneOffset()),'m').utc();
            //var datetime = moment(value, [date_pattern + ' ' + time_pattern, date_pattern_wo_zero + ' ' + time_pattern_wo_zero, moment.ISO_8601], true).tz('UTC');
            if (datetime.isValid() && datetime.year() >= 1900)
                return time.datetime_to_str(datetime.toDate());
            datetime = moment(value, [date_pattern + ' ' + time_pattern, date_pattern_wo_zero + ' ' + time_pattern_wo_zero, moment.ISO_8601]).add(-(new Date().getTimezoneOffset()),'m').utc();
            //datetime = moment.tz(value,'UTC');
            if (datetime.isValid()) {
                if (datetime.year() === 0) {
        ...
        case 'date':
            var date = moment(value, [date_pattern, date_pattern_wo_zero, moment.ISO_8601], true).add(-(new Date().getTimezoneOffset()),'m').utc();
            if (date.isValid() && date.year() >= 1900)
                return time.date_to_str(date.toDate());
            date = moment(value, [date_pattern, date_pattern_wo_zero, moment.ISO_8601]).add(-(new Date().getTimezoneOffset()),'m').utc();
            if (date.isValid()) {
                if (date.year() === 0) {
        ...
        case 'time':
            var _time = moment(value, [time_pattern, time_pattern_wo_zero, moment.ISO_8601], true).add(-(new Date().getTimezoneOffset()),'m').utc();
            if (_time.isValid())
