...
order by a1.cuenta
	...
	""")
		...
		self.env.cr.execute("""
				...
				select cuenta, ABS(SUM(CASE WHEN a1.resultado like '77%' then diferencia else 0 end)) as debit, ABS(SUM(CASE WHEN a1.resultado like '67%' then diferencia else 0 end)) as credit from
		...
		self.env.cr.execute("""
				...
				select resultado, ABS(SUM(CASE WHEN a1.resultado like '67%' then diferencia else 0 end)) as debit, ABS(SUM(CASE WHEN a1.resultado like '77%' then diferencia else 0 end)) as credit from
				view_diferencia_cambio_it a1
